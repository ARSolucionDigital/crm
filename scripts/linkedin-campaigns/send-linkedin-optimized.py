#!/usr/bin/env python3
"""
Optimized version of LinkedIn automation script with improved Big O notation and self-documenting code.
...
class LinkedInCampaignProcessor:
    """
    Main processor class for LinkedIn automation campaigns.
    ...
    def execute_database_query_as_json(self, query):
        ...
        Uses query caching to optimize repeated queries.
    ...
    def execute_raw_database_command(self, sql_command):
        """
        Execute raw SQL statements in the database container.
        Does not cache as these are modification commands.
    ...
    def parse_message_template(self, campaign_name, communication_modality):
        """
    ...
    def personalize_message_template(self, template_text, lead_info):
        """
    ...
    def create_industry_filter_conditions(self, industry):
        """
    ...
    def create_role_filter_conditions(self, role):
        """
    ...
    def fetch_target_leads(
        ...
        if lead_id:
            ...
            if industry_filter:
                industry_conditions = self.create_industry_filter_conditions(industry_filter)
                where_conditions.extend(industry_conditions)

            # Apply role filter with optimized pattern matching
            if role_filter:
                role_conditions = self.create_role_filter_conditions(role_filter)
        ...
        query = f"""
            ...
            LIMIT {limit}
        """
        return self.execute_database_query_as_json(query)

    def update_crm_after_linkedin_interaction(self, lead, campaign_name, action_type, message_sent):
        """
    ...
    def launch_browser_with_stealth_configuration(self, playwright_instance, headless_mode=False):
        """
        ...
        """
        profile_directory = Path(self.config.get("CHROME_PROFILE_DIR", DEFAULT_SESSION_DIR)).expanduser().resolve()
    ...
    def close_any_open_chat_overlays(self, page):
        """
    ...
    def normalize_linkedin_profile_url(self, raw_url):
        """
    ...
    def determine_linkedin_profile_strategy(self, page):
        """
    ...
    def process_lead_interaction(self, page, lead, campaign_name, dry_run_mode, scenario, action_button, communication_modality, subject_rendered, body_rendered):
        """
        ...
        Returns:
            dict: Result of the interaction
        """
        if dry_run_mode:
            print("   🛠️ [DRY-RUN] Simulation successful (Not sent).")
            return {"personId": lead["id"], "name": self.get_lead_name(lead), "status": "DRY_RUN", "scenario": scenario}
        else:
            # Execute actual send based on scenario
            if scenario == "DM":
                if action_button:
                    self.close_any_open_chat_overlays(page)
                    action_button.click()
                    ...
                    if chat_boxes.count() > 0:
                        ...
                        self.simulate_human_typing(target_box, body_rendered)
                ...
                if action_button:
                    ...
                    if subject_inputs.count() > 0 and subject_rendered:
                        ...
                        self.simulate_human_typing(target_subject, subject_rendered)
                        page.wait_for_timeout(800)
                    ...
                    if body_inputs.count() > 0:
                        ...
                        target_body.click()
                        self.simulate_human_typing(target_body, body_rendered)
                        page.wait_for_timeout(1000)
                ...
                if action_button:
                    ...
                    if add_note_btn.count() > 0 and add_note_btn.first.is_visible():
                        ...
                        if note_box.count() > 0:
                            self.simulate_human_typing(note_box.first, body_rendered[:300])
                            page.wait_for_timeout(1000)
            ...
            self.update_crm_after_linkedin_interaction(lead, campaign_name, action_type, body_rendered)
            print(f"   ✅ SENT ({action_type}) & CRM updated.")

            return {
                "personId": lead["id"],
                "name": self.get_lead_name(lead),
...
def perform_session_login_and_setup(config):
    ...
    with sync_playwright() as playwright_instance:
        processor = LinkedInCampaignProcessor(config)
        context = processor.launch_browser_with_stealth_configuration(playwright_instance, headless_mode=False)
...
def test_individual_profile(config, campaign_name, profile_url, dry_run_mode=True):
    ...
    processor = LinkedInCampaignProcessor(config)
    cleaned_url = processor.normalize_linkedin_profile_url(profile_url)
    sample_lead = {
        "nameFirstName": "Laura",
    ...
    with sync_playwright() as playwright_instance:
        ...
        profile_strategy, action_button = processor.determine_linkedin_profile_strategy(page)
        ...
        subject_raw, body_raw = processor.parse_message_template(campaign_name, modality)

        subject_rendered = processor.personalize_message_template(subject_raw, sample_lead)
        body_rendered = processor.personalize_message_template(body_raw, sample_lead)
            ...
            if profile_strategy == "DM":
                if action_button:
                    ...
                    if chat_box.count() > 0:
                        ...
                        processor.simulate_human_typing(chat_box.first, body_rendered)
                ...
                if action_button:
                    processor.close_any_open_chat_overlays(page)
                    ...
                    if subject_inputs.count() > 0 and subject_rendered:
                        ...
                        target_subject = subject_inputs.last
                        target_subject.click()
                        processor.simulate_human_typing(target_subject, subject_rendered)
                        page.wait_for_timeout(800)
                    ...
                    if body_inputs.count() > 0:
                        ...
                        target_body = body_inputs.last
                        target_body.click()
                        processor.simulate_human_typing(target_body, body_rendered)
                        page.wait_for_timeout(1000)
                ...
                if action_button:
                    ...
                    if add_note_button.count() > 0 and add_note_button.first.is_visible():
                        ...
                        if note_box.count() > 0:
                            ...
                            processor.simulate_human_typing(note_box.first, body_rendered[:300])
                            page.wait_for_timeout(1000)
...
def main():
    ...
    config = LinkedInCampaignProcessor({}).load_environment()
    processor = LinkedInCampaignProcessor(config)
    ...
    leads = processor.fetch_target_leads(
    ...
    with sync_playwright() as playwright_instance:
        context = processor.launch_browser_with_stealth_configuration(playwright_instance, headless_mode=is_headless)
        page = context.new_page()

        for i, lead in enumerate(leads, 1):
            lead_name = processor.get_lead_name(lead)
            raw_url = lead.get("linkedinLinkPrimaryLinkUrl", "")
            cleaned_url = processor.normalize_linkedin_profile_url(raw_url)
            ...
            try:
                ...
                scenario, action_button = processor.determine_linkedin_profile_strategy(page)
                ...
                if scenario == "PENDING":
                    ...
                    if not args.dry_run:
                        processor.update_crm_after_linkedin_interaction(lead, args.template, "linkedin.already_pending", "")
                ...
                subject_raw, body_raw = processor.parse_message_template(args.template, communication_modality)

                subject_rendered = processor.personalize_message_template(subject_raw, lead)
                body_rendered = processor.personalize_message_template(body_raw, lead)
                ...
                result = processor.process_lead_interaction(
                    page, lead, args.template, args.dry_run,
    ...
    print(f"📊 Query cache stats: {processor.cache_hits} hits, {processor.cache_misses} misses")
